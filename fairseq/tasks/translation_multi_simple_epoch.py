...
class TranslationMultiSimpleEpochTask(LegacyFairseqTask):
    ...
    def __init__(self, args, langs, dicts, training):
        ...
        # Addd by Eachan: Language-Specific Distillation Weights
        self.LS_distillation_weights = {}
    ...
    def create_batch_sampler_func(
        ...
        def construct_LS_batch_sampler(dataset, epoch):
            ...
            work flow of the original function:
    ...
    def get_batch_iterator(
        ...
        epoch_iter = iterators.EpochBatchIterator(
            dataset=dataset,  # fairseq.data.multilingual.sampled_multi_dataset.SampledMultiDataset
    ...
    def train_step(
        ...
        # 1. get batch language pair:
        batch_lang_pair = self.get_batch_lang_pair(sample)

        # 2. get the language-specific distillation weight
        alpha = self.LS_distillation_weights[batch_lang_pair] \
            if epoch >= self.args.distillation_start_epoch else 0
    ...
    def get_batch_lang_pair(self, sample):
        """
        return the language pair of this batch. I suppose that you use --pure-batch
        """
        return sample["language-pair"]

    def init_LS_distillation_weights(self):
        lang_pairs = self.lang_pairs
        if self.args.mutual_distillation_mode == "automatic":
            for lang_pair in lang_pairs:
                self.LS_distillation_weights[lang_pair] = 0.1
        elif self.args.mutual_distillation_mode == "bidirectional":
            if self.args.pre_LS_distillation_weight is not None:
                pre_LS_distillation_weight = json.loads(self.args.pre_LS_distillation_weight.replace("'", '"'))
                for lang_pair in lang_pairs:
                    self.LS_distillation_weights[lang_pair] = pre_LS_distillation_weight[lang_pair]
            else:
                for lang_pair in lang_pairs:
                    self.LS_distillation_weights[lang_pair] = self.args.distillation_weight
        elif self.args.mutual_distillation_mode == "unidirectional":
            for lang_pair in lang_pairs:
                self.LS_distillation_weights[lang_pair] = 0

            # parse the dataset name

            # dataset = self.args.data.split('/')[-2:]
            # dataset.remove("data-bin")
            # dataset = dataset[0]
            #
            # if dataset == "WMT6-small":
            #     high_resource_langs = ["fr", "de", "zh"]
            #     low_resource_langs = ["tr", "et", "ro"]
            # else:
            #     raise Exception(f"Unknown dataset name {dataset}")
            # high_resource_pairs = [i + '-en' for i in high_resource_langs] + ['en-' + i for i in high_resource_langs]
            # low_resource_pairs = [i + '-en' for i in low_resource_langs] + ['en-' + i for i in low_resource_langs]
            #
            # if self.args.sampling_temperature > 1:  # meaning that the current model is hot model
            #     for lang_pair in high_resource_pairs:
            #         if lang_pair in lang_pairs:
            #             self.LS_distillation_weight[lang_pair] = self.args.distillation_weight
            # else:  # meaning that the current model is cold model
            #     for lang_pair in low_resource_pairs:
            #         if lang_pair in lang_pairs:
            #             self.LS_distillation_weight[lang_pair] = self.args.distillation_weight
