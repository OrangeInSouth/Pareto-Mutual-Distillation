...
class Trainer(object):
    ...
    def get_valid_iterator(
        ...
        first_batch = batch_iterator.first_batch
        self.reset_dummy_batch(first_batch)
        return batch_iterator
    ...
    def train_step(self, samples, raise_oom=False, epoch=-1, step=0, teacher_trainer=None):
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            if getattr(cfg.task, "LS_epoch", None) and (sample is not None and len(sample) > 0):
                ...
                # 1. get language pair of this batch
                batch_lang_pair = sample["language-pair"]

                # 2. modify the logging_output for async:
                for lang_pair in cfg.model.lang_pairs.split(','):
                    logging_output[lang_pair + '_loss'] = torch.zeros_like(_loss)
                    logging_output[lang_pair + '_size'] = 0

                logging_output[batch_lang_pair + '_loss'] = logging_output['loss']
                logging_output[batch_lang_pair + '_size'] = logging_output['sample_size']
        ...
        if getattr(cfg.task, "LS_epoch", None) and (sample is not None and len(sample) > 0):
            ...
            for lang_pair in cfg.model.lang_pairs.split(','):
                LS_valid_logging_output[lang_pair + '_loss'] = logging_outputs[0][
                    lang_pair + '_loss']
                LS_valid_logging_output[lang_pair + '_size'] = logging_outputs[0][
                    lang_pair + '_size']
        ...
        if getattr(cfg.task, "LS_epoch", None) and (sample is not None and len(sample) > 0):
            for lang_pair in cfg.model.lang_pairs.split(','):
                logging_output[lang_pair + '_loss'] = LS_valid_logging_output[
                    lang_pair + '_loss']
                logging_output[lang_pair + '_size'] = LS_valid_logging_output[
                    lang_pair + '_size']
