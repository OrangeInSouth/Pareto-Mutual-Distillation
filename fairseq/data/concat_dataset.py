...
class ConcatDataset(FairseqDataset):
    ...
    def __init__(self, datasets, sample_ratios=1, pure_batch=False, lang_pair_names=None):
        super(ConcatDataset, self).__init__()

        # Added by Eachan:
        self.pure_batch = pure_batch
        if pure_batch:
            print(f"lang_pair_names*****{lang_pair_names}")
            lang_pair_names = [k[k.index(':') + 1:] for k in lang_pair_names]
            self.lang_pair_names = lang_pair_names
    ...
    def collate_with_lang_pair(self, samples, **extra_args):

        batch_samples = self.datasets[0].collater([v for _, v in samples], **extra_args)

        lang_pair_indices = [k for k, _ in samples]
        # print(f"lang_pair_indices-1 | {lang_pair_indices}")

        if len(list(set(lang_pair_indices))) > 1:
            print("This validation batch contains data from multiple language pairs.")
            print(f"language pair numbers: {len(list(set(lang_pair_indices)))}")
            exit(0)

        # print(f"lang_pair_indices-2 | {lang_pair_indices}")

        maybe_lang_pair_index = lang_pair_indices[0]
        # print(maybe_lang_pair_index)
        batch_samples["language-pair"] = self.lang_pair_names[maybe_lang_pair_index]
        return batch_samples

    def collater(self, samples, **extra_args):
        # For now only supports datasets with same underlying collater implementations
        if hasattr(self.datasets[0], "collater"):
            if self.pure_batch and len(samples) > 0:
                return self.collate_with_lang_pair(samples, **extra_args)
            else:
    ...
    def ordered_indices(self):
        ...
        Re-implemented by Eachan:
            I want this function to return a list, whose length is the number of self.datasets
