...
def default_virtual_size_func(datasets, ratios, max_scale_up=1.0):
    """
    计算数据集的虚拟大小，我将max_scale_up从1.5变成了1
    """
...
class SampledMultiDataset(FairseqDataset):
    ...
    def __init__(
        ...
        # self.keys里面放了各个language-pair的名字
        # e.g. ['main:bos-eng', 'main:mar-eng', 'main:hin-eng', 'main:mkd-eng', 'main:ell-eng',
        # 'main:bul-eng', 'main:fra-eng', 'main:kor-eng']
    ...
    def collater(self, samples, **extra_args):
        ...
        if self.collate_format == CollateFormat.ordered_dict:
            ...
            # added by Eachan: add the language pair name of this batch samples into the `batch`
            if self.pure_batch:
                language_pairs = [i[0] for i in samples]
                assert len(set(language_pairs)) == 1, "this batch contain data from multiple language pairs."
                lang_pair = self.keys[language_pairs[0]]
                batch["language-pair"] = lang_pair[lang_pair.index(':') + 1:]
        else:
