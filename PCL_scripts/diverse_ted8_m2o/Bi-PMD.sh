...
lang_list=/userhome/ychuang/online_distillation_MNMT/lang_list_diverse.txt
lang_list=/userhome/ychuang/online_distillation_MNMT/lang_list_diverse.txt
...
SAVE_DIR=${project_path}/checkpoints/diverse_ted8_m2o/Bi-PMD
mkdir -vp $SAVE_DIR
...
${python_path} ${project_path}/fairseq_cli/train.py $path_2_data \
  ...
  --sampling-temperature-2 3 \
  --mutual-distillation-mode "bidirectional" \
  --distillation-weight 0.8 \
  ...
  --max-epoch 200 --max-update 200000 \
...
bash ${project_path}/PCL_scripts/diverse_ted8_m2o/test.sh ${SAVE_DIR}/model1
bash ${project_path}/PCL_scripts/diverse_ted8_m2o/test.sh ${SAVE_DIR}/model2
